    ...
    for (const item of items) {
        ...
        switch (type) {
            case 'bounce': {
                ...
                await recordBounce(email, bounceType, diagnosticCode, source, authResult.orgId);
            ...
            case 'complaint': {
                ...
                await recordComplaint(email, feedbackType, source, authResult.orgId);
                results.push({ email, action: 'complained', reason: feedbackType ?? 'abuse' });
                break;
            }

            case 'invalid': {
                await addSuppression(email, 'invalid_address', (item.source as string) ?? 'validation', undefined, authResult.orgId);
...
export async function GET(request: NextRequest): Promise<Response> {
    ...
    const stats = await getSuppressionStats(authResult.orgId);

    return NextResponse.json({
        success: true,
        stats,
        ...(includeList ? { suppressions: await getAllSuppressions(authResult.orgId) } : {}),
    });
}
